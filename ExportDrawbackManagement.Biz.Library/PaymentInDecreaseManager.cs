...
{
	class PaymentInDecreaseManager : BaseManager<T_PaymentIndecreaseHead>, IPaymentInDecreaseManager
	{
		public void insertInDecreaseList(DataTable dt)
		{
			Database db = Dao.GetDatabase();
			string sql = @"INSERT INTO [dbo].[payment_Indecrease_list]
							(
								[bill_no],
								[g_no],
								[amount],
		...
		public DataSet getListsAll()
		{
            ...
            string sql = @"SELECT  a.*,b.check_status
                            FROM    [dbo].[payment_Indecrease_list] a ,
		...
		{
			Database db = Dao.GetDatabase();
			string sql = @"DELETE FROM [dbo].[payment_Indecrease_list] WHERE bill_no = @bill_no and g_no = @g_no ";
			using (DbConnection cn = db.CreateConnection())
			{
				try
				{
		...
		{
			Database db = Dao.GetDatabase();
			string sql = @"DELETE FROM [dbo].[payment_Indecrease_head] WHERE bill_no = @bill_no";
			using (DbConnection cn = db.CreateConnection())
			{
				try
				{
		...
		{
			Database db = Dao.GetDatabase();
			string sql = @"SELECT * FROM [dbo].[payment_Indecrease_head] where  bill_no = @bill_no";
			using (DbConnection cn = db.CreateConnection())
			{
				try
				{
		...
		public void addInDecreaseHead(T_PaymentIndecreaseHead head)
		{
			...
			Database db = Dao.GetDatabase();
			string sql = @"INSERT INTO [dbo].[payment_Indecrease_head]
							   ([bill_no]
							   ,[itemID]
							   ,[customerID]
							   ,[customer]
		...
		{
			Database db = Dao.GetDatabase();
			string sql = @"UPDATE [dbo].[payment_Indecrease_head]
						   SET [itemID] = @itemID
		...
		public string getLastBillNo(string key)
		{
			...
			string sql = @"SELECT * FROM [dbo].[payment_Indecrease_head] where  bill_no like @bill_no order by bill_no desc";
			using (DbConnection cn = db.CreateConnection())
			{
				try
				{
        ...
        public DataSet getUnCheckInDecreaseLists(int CustID,int currencyID)
        {
            ...
            DataSet ds;
            string sql = @"SELECT  *
                            FROM    [dbo].[payment_Indecrease_head]
		...
		{
			Database db = Dao.GetDatabase();
			string sql = @"SELECT * FROM [dbo].[payment_Indecrease_list] where  bill_no = @bill_no";
			using (DbConnection cn = db.CreateConnection())
			{
				try
				{
